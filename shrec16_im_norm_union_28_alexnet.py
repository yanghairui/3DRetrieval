...
from dataset.shrec_2016 import shrec_3DSketch
...
parser = argparse.ArgumentParser(description='SHREC 2016 - 3D Sketch based 3D Object Retrieval')
parser.add_argument('--config', default='experiments/shrec_2016/config.yaml')
parser.add_argument('--center_path', default='experiments/shrec_2013/model/alexnet/checkpoints/AlexNet50all.pth')
# parser.add_argument('--center_path', default='pkl/alexnet_sub_centers')
...
ImageTrainSet = shrec_3DSketch(path=config.PATH, phase='train', transform=transform_train)
...
ImageValSet = shrec_3DSketch(path=config.PATH, phase='test', transform=transform_val)
...
# tcl_wo_params_loss = CenterNParamsLoss.CenterWoParamMultiCosineNearLoss(
#     centers_path=args.center_path,
#     l2Norm=l2.L2Norm())
...
def train_one_epoch(epoch_idx):
    ...
    for batch_idx, item in enumerate(ImageTrainLoader):
        images, labels, map_labels = item
        images = images.cuda()
        labels = labels.type(torch.long).cuda()
        map_labels = map_labels.type(torch.long).cuda()
        ...
        tcl_loss = tcl_wo_params_loss(feats, map_labels)
...
def save_checkpoints(accuracy, mAP, epoch_idx, surffix=''):
    ...
    torch.save(
        infos,
        os.path.join(config.save_path, config.CHECK_POINTS, 'AlexNet' + str(config.MODEL_TYPE) + surffix + '.pth'))
